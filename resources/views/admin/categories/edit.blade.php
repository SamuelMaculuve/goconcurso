...
@section('page-subtitle', 'Actualizar: ' . ($category->name ?? ''))
            ...
            <div>
                ...
                <div class="flex items-center gap-3">
                    <input type="color" id="color" name="color"
                           value="{{ old('color', $category->color ?? '#C0602A') }}"
                           x-model="colorValue"
...
function categoryEditForm() {
    return {
        ...
        iconPreview: '{{ old('icon', $category->icon ?? '📁') }}',
        colorValue: '{{ old('color', $category->color ?? '#C0602A') }}',
        updateSlug(name) {
            this.slug = name
                .toLowerCase()
                .normalize('NFD')
